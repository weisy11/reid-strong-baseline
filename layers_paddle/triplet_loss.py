...
class TripletLoss(object):
    ...
    def __init__(self, margin=None):
        ...
        if margin is not None:
            self.ranking_loss = paddle.nn.MarginRankingLoss(margin=margin)
        # else:
        #     self.ranking_loss = paddle.nn.SoftMarginLoss()
...
class CrossEntropyLabelSmooth(paddle.nn.Layer):
    ...
    def forward(self, inputs, targets):
        ...
        targets = paddle.zeros(log_probs.size()).scatter_(1, targets.unsqueeze(1).data.cpu(), 1)
        if self.use_gpu:
